...
export const ContainerScroll = ({
  ...
  const scaleDimensions = () => {
    return isMobile ? [0.7, 0.9] : [1.75, 1.7];
  };

  const rotate = useTransform(scrollYProgress, [0, 1], [30, 0]);
  const scale = useTransform(scrollYProgress, [0, 1], scaleDimensions());
  const translate = useTransform(scrollYProgress, [0, 1], [-400, -100]);

  return (
    <div
      className="h-[160rem] flex items-center justify-center relative p-20"
      ref={containerRef}
    >
      <div
